...
(function () {
  ...
  Dot.prototype.draw = function () {
    ...
    if (this.r > 1.0) {
      ...
      grd.addColorStop(0, 'rgba(143,226,138,' + (this.a * .7) + ')');
      grd.addColorStop(1, 'rgba(143,226,138,0)');
      ctx.beginPath();
    ...
    for (var i = 0; i < pts.length; i++) {
      for (var j = i + 1; j < pts.length; j++) {
        ...
        if (dist < maxDist) {
          ...
          ctx.strokeStyle = 'rgba(143,226,138,' + alpha + ')';
          ctx.lineWidth   = 1.1;
          ctx.stroke();
        }
      }
...
/* ═══════════════════════════════
   ...
   ═══════════════════════════════ */
(function () {
  var words = ['Hogar', 'Negocio', 'Transporte', 'Bodega'];
  var idx   = 0;
...
document.getElementById('cForm').addEventListener('submit', function (e) {
  ...
  var txt = '*Solicitud de Cotización — KOG Control de Plagas*\n\n';
  txt += '• Nombre: '    + nombre  + '\n';
  if (empresa) txt += '• Empresa/Negocio: ' + empresa + '\n';
  txt += '• Teléfono: '  + tel     + '\n';
  if (email)   txt += '• Correo: '  + email   + '\n';
  txt += '• Tipo de servicio: ' + tipo    + '\n';
  txt += '• Descripción: '      + mensaje;

  window.open('https://wa.me/525643096048?text=' + encodeURIComponent(txt), '_blank');
});
